...
高级建模实例：[http://127.0.0.1:4224](http://127.0.0.1:4224)，包含车辆、飞行器、建筑与曲面道具四个可编辑工程。新增拓扑、曲面、修改器及模型导出能力见 [高级建模入口](docs/advanced-modeling-delivery.md) 和 [MCP 使用说明](docs/advanced-modeling-mcp.md)。Web 与外部 Agent 共用领域操作；不内置聊天或接入生成式 3D、下游 AI 视频服务。

上一目标的四部两分钟影片及原交付实例 [http://127.0.0.1:4219](http://127.0.0.1:4219) 保留，文件和验收见 [原交付入口](docs/delivery-runtime.md)。新增高级建模目标扩展了几何编辑范围，雕刻、专业材质、布料、毛发和流体仍不纳入。
...
- 稳定顶点/边/面组件、真实视口拾取与框选/套索、比例变换、区域挤出、内插、多段倒角、环切/滑移、剖切、桥接/补洞及网格诊断修复。
- 活布尔依赖、实体化、焊接镜像、曲线阵列、弯曲/扭转和 Catmull-Clark；Bezier 自定义截面扫掠、旋转成型和多截面放样；静态 GLB 显式转拓扑、所选对象或场景 GLB 导出。
- 可取消的后台建模、诊断、转换和模型导出，独立 Worker 处理几何与组件准备；SQLite 原子提交和跨重启项目恢复。
